...
export class AuthService {  
  ...
  auth0Client$ = (from(
    ...
    // Every subscription receives the same shared value (the last auth0Client)
    shareReplay(1), 
  ...
  isAuthenticated$ = this.auth0Client$.pipe(
    ...
    tap(temp => console.log('isAuthenticated$ promise ran'))
  );
  ...
  localAuthSetup() {
    ...
    const checkAuth$ = this.isAuthenticated$.pipe(
      concatMap((loggedIn: boolean) => {
        console.log('localAuthSetup promise ran');
        if (loggedIn) {
